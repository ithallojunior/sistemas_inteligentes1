...
class mlp():
    ...
    max_iter=1000, bias=True, tol=1e-3, seed=None, keep_error_list=True, warm_start=False, coefs=None, weight_range=(0.,1.), classifier=False):
    ...
    def predict(self, X):
        ...
        # case for just predict, with any size, local bias

        if self.bias:
            local_bias = np.ones((X.shape[0], 1)) # vector of ones for the bias
        else:
            local_bias = np.zeros((X.shape[0], 1)) # vector of ones for the bias

        #bias in the network, always there, sometimes as zeroes

        layer0 = np.hstack((X, local_bias))

        layer1 = self._function(np.dot(layer0, self.coefs[0]))

        layer1_w_bias = np.hstack((layer1, local_bias))

        layer2 = self._function(np.dot(layer1_w_bias, self.coefs[1]))
        #case classification
        if self.classifier:
            layer2  = np.where(layer2 >=0.5, 1., 0.)
    ...
    def score(self, X, y):
        ...
        y = self._y_shape_corrector(y)
        y_ = self.predict(X)
        #different scores for classification and regression
        if self.classifier:
            total_corrects = np.count_nonzero(np.equal(y_, y))
            return total_corrects/float(X.shape[0])
        else:
...
if __name__=="__main__":
    clf = mlp(seed=1, activation="tanh", max_iter=10000,
          hidden_layer_size=4, alpha=0.1, momentum=0.9,tol=1e-3,
          weight_range=(-1,1), bias=True, classifier=True)
    clf.example_run(show="online")
    print clf.predict(np.array([[0,1], [0,0]]))
